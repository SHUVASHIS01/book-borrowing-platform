...
function AnimatedNumber({ value, suffix = "" }: { value: number; suffix?: string }) {
  ...
  return (
    <animated.span>
      {spring.val.to((v) => `${Math.floor(v)}${suffix}`)}
    </animated.span>
  );
}
...
export default function Home() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 text-white overflow-hidden min-h-[85vh] flex items-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <div className="flex items-center gap-8 mt-12 pt-8 border-t border-white/10 animate__animated animate__fadeIn">
                <div>
                  <p className="text-2xl font-bold text-white"><AnimatedNumber value={1200} suffix="+" /></p>
                  <p className="text-sm text-slate-400">Books Available</p>
                </div>
                <div className="w-px h-10 bg-white/10" />
                <div>
                  <p className="text-2xl font-bold text-white"><AnimatedNumber value={500} suffix="+" /></p>
            ...
            <div className="hidden lg:block relative animate__animated animate__fadeInRight">
              <div className="relative">
                ...
                <div className="absolute -top-10 right-10 w-20 h-20 bg-gradient-to-br from-blue-500/20 to-indigo-500/20 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10">
                ...
                <div className="absolute -bottom-4 left-8 w-16 h-16 bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10">
                  <svg className="w-6 h-6 text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Marquee */}
      <div className="relative bg-slate-900 border-y border-slate-800 overflow-hidden">
        <div className="flex items-center">
          <div className="marquee whitespace-nowrap py-4 flex items-center gap-8 text-sm">
            {[...Array(2)].map((_, idx) => (
              <div key={idx} className="flex items-center gap-8">
                <span className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-blue-500 rounded-full" />
                  <span className="text-white font-semibold">New Arrivals</span>
                  <span className="text-slate-400">The Great Gatsby</span>
                </span>
                <span className="flex items-center gap-2">
                ...
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-slate-400">Cosmos by Carl Sagan</span>
                </span>
                <span className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
                  <span className="text-white font-semibold">Must Read</span>
                  <span className="text-slate-400">Sapiens</span>
                </span>
                <span className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-amber-500 rounded-full" />
                  <span className="text-white font-semibold">Classic</span>
                  <span className="text-slate-400">1984 by George Orwell</span>
                </span>
                <span className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-rose-500 rounded-full" />
                  <span className="text-white font-semibold">Featured</span>
                  <span className="text-slate-400">The Alchemist</span>
                </span>
              </div>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {featuredBooks.map((book, i) => (
                <BookCard key={book._id} book={book} buttonLabel="View Details" index={i} />
              ))}
            </div>
          )}
        </div>
      ...
      <section className="py-20 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: "Story", desc: "Fiction, Fantasy & Adventure", count: "500+", icon: "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253", bg: "bg-amber-50" },
              { name: "Tech", desc: "Programming & Development", count: "300+", icon: "M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4", bg: "bg-blue-50" },
              { name: "Science", desc: "Physics, Biology & Space", count: "400+", icon: "M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z", bg: "bg-emerald-50" },
            ].map((cat) => (
              <Link
                key={cat.name}
              ...
              >
                <div className={`w-14 h-14 rounded-2xl ${cat.bg} flex items-center justify-center mb-5 group-hover:scale-110 transition-transform`}>
                  <svg className="w-7 h-7 text-slate-700" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d={cat.icon} /></svg>
                </div>
      ...
      <section className="py-20 md:py-24 bg-gradient-to-b from-slate-50 to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: "Instant Access", desc: "Borrow books instantly with just one click", icon: "M13 10V3L4 14h7v7l9-11h-7z", bg: "bg-amber-50" },
              { title: "Read Anywhere", desc: "Access your borrowed books from any device", icon: "M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z", bg: "bg-blue-50" },
              { title: "Premium Collection", desc: "Curated selection of top-quality books", icon: "M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z", bg: "bg-purple-50" },
              { title: "Easy Returns", desc: "Hassle-free return process when you are done", icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15", bg: "bg-emerald-50" },
            ].map((item, i) => (
              <div key={i} className="group text-center p-7 rounded-2xl border border-slate-200/60 bg-white hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300">
